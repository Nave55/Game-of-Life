class ConwayRaylib {
    ...
    static private function drawCells(): Void {
        for (row in 0...ROWS) {
            for (column in 0...COLS) {
                var color = cells[row][column] == 1 ? Clr.green() : Clr.darkGrey();
                Rl.drawRectangle(
                    (column * CELL_SIZE), 
                    (row * CELL_SIZE), 
                    (CELL_SIZE - 1), 
    ...
    static private function fillRandom(): Void {
        if (!running) {
            for (row in 0...ROWS) {
                for (column in 0...COLS) {
    ...
    static private function clearGrid(): Void {
        if (!running) {
            for (row in 0...ROWS) {
                for (column in 0...COLS) {
                    cells[row][column] = 0;
    ...
    static private function countLiveNbrs(row: Int, col: Int): Int {
    ...
    static private function updateSim(): Void {
        if (running) {
            for (row in 0...ROWS) {
                for (column in 0...COLS) {
    ...
    static private function gameControls(): Void {
    ...
    static private function drawGame() {
        Rl.beginDrawing();
    ...
    static private function updateGame(): Void {
        gameControls();
        updateSim();
        drawGame();
