...
fn each_cell(rows int, cols int) []Cell {
	mut out := []Cell{}
	for r in 0 .. rows {
		for c in 0 .. cols {
			out << Cell{r, c}
...
fn fill_random(mut cells [][]int, running bool) {
	if !running {
		for i in each_cell(rows, cols) {
			cells[i.r][i.c] = if rl.get_random_value(0, 3) == 1 { 1 } else { 0 }
		}
	}
}
...
fn draw_cells(mut cells [][]int) {
	for i in each_cell(rows, cols) {
		rl.draw_rectangle(i32(i.c * cell_size), i32(i.r * cell_size), i32(cell_size - 1),
			i32(cell_size - 1), if cells[i.r][i.c] == 1 { rl.green } else { dark_grey })
	}
}
...
fn update_sim(mut game Game) {
	if game.running {
		for i in each_cell(rows, cols) {
			live_nbrs := count_live_nbrs(mut game.cells, i.r, i.c)
			cell_value := game.cells[i.r][i.c]

			if cell_value == 1 {
				game.tmp_cells[i.r][i.c] = if live_nbrs > 3 || live_nbrs < 2 { 0 } else { 1 }
			} else {
				game.tmp_cells[i.r][i.c] = if live_nbrs == 3 { 1 } else { 0 }
			}
		}

		for i in each_cell(rows, cols) {
			game.cells[i.r][i.c] = game.tmp_cells[i.r][i.c]
		}
	}
}
...
fn (mut game Game) update_game() {
	...
	if game.running {
		rl.set_window_title('Game of Life is Running at ${game.fps}')
	} else {
