...
fn fill_random(mut cells [][]int, running bool) {
	if !running {
		for r in 0 .. rows {
			for c in 0 .. cols {
				cells[r][c] = if rl.get_random_value(0, 3) == 1 { 1 } else { 0 }
			}
		}
	}
}
...
fn draw_cells(mut cells [][]int) {
	for r in 0 .. rows {
		for c in 0 .. cols {
			rl.draw_rectangle(i32(c * cell_size), i32(r * cell_size), i32(cell_size - 1),
				i32(cell_size - 1), if cells[r][c] == 1 { rl.green } else { dark_grey })
		}
	}
}
...
fn update_sim(mut game Game) {
	if game.running {
		for r in 0 .. rows {
			for c in 0 .. cols {
				live_nbrs := count_live_nbrs(mut game.cells, r, c)
				cell_value := game.cells[r][c]

				if cell_value == 1 {
					game.tmp_cells[r][c] = if live_nbrs > 3 || live_nbrs < 2 { 0 } else { 1 }
				} else {
					game.tmp_cells[r][c] = if live_nbrs == 3 { 1 } else { 0 }
				}
			}
		}
		...
		for r in 0 .. rows {
			for c in 0 .. cols {
				game.cells[r][c] = game.tmp_cells[r][c]
			}
		}
	}
}
